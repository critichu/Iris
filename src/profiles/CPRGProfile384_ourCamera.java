...
import tileReaders.BasicTileReader_simpleEmptyCheck;
 ...
 *
 */
public class CPRGProfile384_ourCamera extends Profile {

	/**
	 * the user-friendly name of this profile (will appear in the drop-down list of the GUI) 
	 */
	public static String profileName = "CPRG readout Profile for 384 plates _2";
	...
	public void analyzePicture(String filename){
		...
		croppedImage = colorCroppedImage.duplicate();
		ImageConverter imageConverter = new ImageConverter(croppedImage);
		imageConverter.convertToGray8();
		ImagePlus grayscaleImage = croppedImage.duplicate();
		turnImageBW_auto(croppedImage);
		...
		BasicImageSegmenterInput segmentationInput = new BasicImageSegmenterInput(grayscaleImage, settings);
		...
		for(int i=0;i<settings.numberOfRowsOfColonies;i++){
			...
			for (int j = 0; j < settings.numberOfColumnsOfColonies; j++) {
				...
				basicTileReaderOutputs[i][j] = BasicTileReader_simpleEmptyCheck.processTile(
		...
		if(checkRowsColumnsIncorrectGridding(basicTileReaderOutputs)){
			...
			colorCroppedImage = ColonyBreathing.paintSegmentedImage(colorCroppedImage, segmentationOutput);
