...
public class CPRGImageCropper {
	 ...
	 * Modifies the input picture.
	 * @param originalImage
	 * @return
	 */
	private static Roi findPlatePlasticBorders(ImagePlus originalImage) {

		//0. before we convert it to grayscale, remove red and blue, keep only green (which is the background of the plate)
		originalImage = keepJustGreen(originalImage);		
		...
		int columnsStartArea = width/2 - 200;
		int columnsEndArea = width/2 + 200;
		...
		//define from where to where to get the sums of brightness (in the sawtooth pattern)
		int rowsStartArea = height/2 - 150;
		int rowsEndArea = height/2 + 150;
		//get the sublist of those sums
		...
		int indexOfLeftBorder = indicesOfColumnsSumsAboveMean.get(skipX);
		int indexOfRightBorder = indicesOfColumnsSumsAboveMean.get(indicesOfColumnsSumsAboveMean.size()-skipX);

		//get the 20th from the top and the 20th from the bottom
		int indexOfTopBorder = indicesOfRowsSumsAboveMean.get(skipY);
		int indexOfBottomBorder = indicesOfRowsSumsAboveMean.get(indicesOfRowsSumsAboveMean.size()-skipY);
	...
	/**
	 * This function will keep just the green color and return a copy of the original image.
	 * Does not modify the input image.
	 * @param originalImage
	 * @return
	 */
	private static ImagePlus keepJustGreen(ImagePlus input) {

		ImagePlus editedImage = input.duplicate();
		
		ColorProcessor processor = (ColorProcessor) editedImage.getProcessor();


		//set channels Red and Blue (1 and 3) to zero
		processor.setChannel(1, new ByteProcessor(processor.getWidth(), processor.getHeight()));
		processor.setChannel(3, new ByteProcessor(processor.getWidth(), processor.getHeight()));


		editedImage.updateImage();
