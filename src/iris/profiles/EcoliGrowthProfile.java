...
import iris.tileReaders.BasicTileReader;
 ...
 *
 */
public class EcoliGrowthProfile extends Profile {

	/**
	 * the user-friendly name of this profile (will appear in the drop-down list of the GUI) 
	 */
	private static String profileName = "E.coli Profile";


	/**
	 * this is a description of the profile that will be shown to the user on hovering the profile name 
	 */
	public static String profileNotes = "This profile is calibrated for use in measuring the colony sizes of E. coli on the UCSF screens";
	...
	public void analyzePicture(String filename){
		...
		if(originalImage==null){
			...
			System.err.println("Could not open image file: " + justFilename);
			return;
		}
		//
		//--------------------------------------------------
		...
		if(segmentationOutput.errorOccurred){
			...
			System.err.println("\nE.coli growth profile: unable to process picture " + justFilename);
		...
		for(int i=0;i<settings.numberOfRowsOfColonies;i++){
			//for all columns
			for (int j = 0; j < settings.numberOfColumnsOfColonies; j++) {
				output.append(Integer.toString(i+1) + "\t" + Integer.toString(j+1) + "\t" + Integer.toString(readerOutputs[i][j].colonySize) + "\n");
			}
		}
