/*
 *  G.Landini at bham dot ac dot uk
 *
 *  Copyright (C) 2001-2010 by G.Landini
 *
...
public class AutoThresholder_IMMI {
	...
	private static final int HUANG = 0, INTERMODES = 1, ISODATA = 2, LI = 3,
			MAXENTROPY = 4, MEAN = 5, MINERROR = 6, MINIMUM = 7, MOMENTS = 8,
			OTSU = 9, PERCENTILE = 10, RENYIENTROPY = 11, SHANBHAG = 12,
			TRIANGLE = 13, YEN = 14;
	...
	private boolean bimodalTest(double[] y) {
		int len = y.length;
		boolean b = false;
		int modes = 0;
	...
	public int getThreshold(String method, int[] histogram) {
		...
		case LI:
			return Li(histogram);
		case MAXENTROPY:
			return MaxEntropy(histogram);
		case MEAN:
			return Mean(histogram);
		case MINERROR:
			return MinErrorI(histogram);
		case MINIMUM:
		...
		case RENYIENTROPY:
			return RenyiEntropy(histogram);
		case SHANBHAG:
			return Shanbhag(histogram);
		case TRIANGLE:
			return Triangle(histogram);
		case YEN:
			return Yen(histogram);
		}
		return -1;
	}

	private int Huang(int[] data) {
	...
	private int Intermodes(int[] data) {
	...
	private int IsoData(int[] data) {
		...
		//
		// However the code commented below have the occasional discrepancy with
		// IJ code.
		...
		return level;
		/*
		 * // this should work straight away, but for some images there is a
		 * discrepancy with IJ int i, l, toth, totl, h, g=0; for (i = 1; i <
		 * 256; i++){ if (data[i] > 0){ g = i + 1; break; } } while (true){ l =
		 * 0; totl = 0; for (i = 0; i < g; i++) { totl = totl + data[i]; l = l +
		 * (data[i] * i); } h = 0; toth = 0; for (i = g + 1; i < 256; i++){ toth
		 * += data[i]; h += (data[i]*i); } if (totl > 0 && toth > 0){ l /= totl;
		 * h /= toth; if (g == (int) Math.round((l + h) / 2.0)) break; } g++; if
		 * (g > 254) return -1; } return g+1; // +1 to match ImageJ output, but
		 * some images are off by +1 or -1
		 */
	}

	private int Li(int[] data) {
	...
	private int MinErrorI(int[] data) {
		...
		int threshold = Mean(data); // Initial estimate for the threshold is
	...
	private int RenyiEntropy(int[] data) {
		// Kapur J.N., Sahoo P.K., and Wong A.K.C. (1985) "A New Method for
		// Gray-Level Picture Thresholding Using the Entropy of the Histogram"
		// Graphical Models and Image Processing, 29(3): 273-285
		// M. Emre Celebi
