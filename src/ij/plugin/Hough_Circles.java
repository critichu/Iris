...
public class Hough_Circles implements PlugInFilter {
	...
	int radius[]; // Radius  ///Added by George Kritikos 16.05.2014
	...
	public Ellipse2D my_run(ImageProcessor ip){
		...
		//put the parameters of Hough here
		houghTransform();

		getCenterPoints(maxCircles);

		//this is the point where we should have the maximum radius and best Point returned as a Roi...
...
//		int maxIndex = 0;
	...
	private void getCenterPoints (int maxCircles) {
		...
		for(int c = 0; c < maxCircles; c++) {
			...
			radius[c] = rMax; ///Added by George Kritikos 16.05.2014
