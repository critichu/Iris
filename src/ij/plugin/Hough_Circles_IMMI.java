 ...
 */
public class Hough_Circles_IMMI implements PlugInFilter {

	public int radiusMin = -1; // Find circles with radius grater or equal
	// radiusMin
	public int radiusMax = -1; // Find circles with radius less or equal
	// radiusMax
	public int radiusInc = -1; // Increment used to go from radiusMin to
	// radiusMax
	public int maxCircles = 1; // Numbers of circles to be found
	...
	public int width = -1; // Hough Space width (depends on image width)
	public int height = -1; // Hough Space heigh (depends on image height)
	public int depth = -1; // Hough Space depth (depends on radius interval)
	public int offset = -1; // Image Width
	public int offx = -1; // ROI x offset
	public int offy = -1; // ROI y offset
	...
	private int buildLookUpTable() {
		...
		for (int radius = radiusMin; radius <= radiusMax; radius = radius
			...
			for (int incNun = 0; incNun < incDen; incNun++) {
				double angle = (2 * Math.PI * incNun) / incDen;
				int indexR = (radius - radiusMin) / radiusInc;
				int rcos = (int) Math.round(radius * Math.cos(angle));
				int rsin = (int) Math.round(radius * Math.sin(angle));
	...
	private void clearNeighbours(int x, int y, int radius) {
		...
		int y1 = (int) Math.floor(y - halfRadius);
		int y2 = (int) Math.ceil(y + halfRadius) + 1;
		int x1 = (int) Math.floor(x - halfRadius);
		int x2 = (int) Math.ceil(x + halfRadius) + 1;
	...
	public void run(ImageProcessor ip) {
		...
		if (maxCircles != -1 || maxCircles == -1) {
			houghTransform();

			// Create image View for Hough Transform.
			ImageProcessor newip = new ByteProcessor(width, height);
			// ImageProcessor newip = ip;
			byte[] newpixels = (byte[]) newip.getPixels();
			createHoughPixels(newpixels);

			// copy result
			byte[] result = (byte[]) ip.getPixels();
			byte[] pixels = (byte[]) newip.getPixels();
			for (int h = 0; h < pixels.length; h++)
				result[h] = pixels[h];
			// ImagePlus display = new ImagePlus("Hough Space [r=" + radiusMin
			// + "]", newip);
			// Shows only the hough space for the minimun radius
			// if (display != null) {
			// display.hide();
			// display.close();
			// }
			// display = new ImagePlus(maxCircles + " Circles Found", newip);
			// display.show();
		}
	}
	...
	public int setup(String arg, ImagePlus imp) {
		...
		radiusMin = Integer.parseInt(args[0]);
		radiusMax = Integer.parseInt(args[1]);
		radiusInc = Integer.parseInt(args[2]);
