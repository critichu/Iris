...
public class A_trous_filter implements PlugInFilter {
	...
	public void a_trous_transform(double[][] matrix_big,
			double[][] matrix_big2, double[][][] wave3d, int width, int height,
			int m) {
		...
		for (int k = 0; k < 5; k++) {
			...
			convolution(matrix_big, matrix_big2, distance[k], distance[k + 1],
					width, height, m);
	...
	public void convolution(double[][] mat1, double[][] mat2, int p, int q,
			int width, int height, int m) {

		for (int i = m; i < width + m; i++) {
			for (int j = m; j < height + m; j++) {

				mat2[i][j] = (mat1[i - q][j - q] / 256
						+ mat1[i - q][j - p] / 64 + mat1[i - q][j] * 3 / 128
						+ mat1[i - q][j + p] / 64 + mat1[i - q][j + q] / 256
						+ mat1[i - p][j - q] / 64 + mat1[i - p][j - p] / 16
						+ mat1[i - p][j] * 3 / 32 + mat1[i - p][j + p] / 16
						+ mat1[i - p][j + q] / 64 + mat1[i][j - q] * 3 / 128
						+ mat1[i][j - p] * 3 / 32 + mat1[i][j] * 9 / 64
						+ mat1[i][j + p] / 32 + mat1[i][j + q] * 3 / 128
						+ mat1[i + p][j - q] / 64 + mat1[i + p][j - p] / 16
						+ mat1[i + p][j] * 3 / 32 + mat1[i + p][j + p] / 16
						+ mat1[i + p][j + q] / 64 + mat1[i + q][j - q] / 256
						+ mat1[i + q][j - p] / 64 + mat1[i + q][j] * 3 / 128
						+ mat1[i + q][j + p] / 64 + mat1[i + p][j + q] / 256);
	...
	// remove the noise
	public void denoise(double[][][] wave, double[][][] wave_s, int width,
			int height, int m, double stddev, double k1, double k2, double k3,
			double k4, double k5) {
		...
		/*
		 * for(int i=0;i<5;i++){ result=IJ.d2s(dev_s[i]); IJ.write(result); }
		 */

		for (int i = 0; i < width + m * 2; i++) {
			for (int j = 0; j < height + m * 2; j++) {

				if (Math.abs(wave[i][j][0]) < k1 * stddev * dev_s[0])
					wave[i][j][0] = 0;
				if (Math.abs(wave[i][j][1]) < k2 * stddev * dev_s[1])
					wave[i][j][1] = 0;
				if (Math.abs(wave[i][j][2]) < k3 * stddev * dev_s[2])
					wave[i][j][2] = 0;
				if (Math.abs(wave[i][j][3]) < k4 * stddev * dev_s[3])
					wave[i][j][3] = 0;
				if (Math.abs(wave[i][j][4]) < k5 * stddev * dev_s[4])
					wave[i][j][4] = 0;
	...
	public void disp_noise(int width, int height, double[] pix_d, byte[] save) {
		...
		ImagePlus noise = NewImage.createByteImage("Removed noise", width,
		...
		for (int d = 0; d < width * height; d++)
			rem_noise_d[d] = save_d[d] - pix_d[d];

		for (int f = 0; f < width * height; f++) {
			if (rem_noise_d[f] < 0)
				rem_noise_d[f] = 0;
			if (rem_noise_d[f] > 255)
				rem_noise_d[f] = 255;
		}

		for (int d = 0; d < width * height; d++) {
			save[d] = (byte) rem_noise_d[d];
	...
	public void filter_poisson(int width, int height, double devp,
			double[] pix_d, byte[] pixels) {

		int pos;
		double dev;
		double mean;
		double[][] tmp_mat = new double[width + 2][height + 2]; // temporary
		// matrix for
		// edge effets
		...
		for (int i = 0; i < width; i++)
			for (int j = 0; j < height; j++)
				tmp_mat[i + 1][j + 1] = mat[i][j]; // copy the matrix in the
		// center of the tmp matrix
		...
		for (int i = 1; i <= width; i++) { // scan of the matrix
			for (int j = 1; j <= height; j++) {
				...
				for (int x = i - 1; x <= i + 1; x++) {
					for (int y = j - 1; y <= j + 1; y++) { // copy of the 3x3
						// neighborhood
				...
				dev = stdev_calc(tmp);
				if (fish[4] > mean + devp * dev || fish[4] < mean - devp * dev)
					tmp_mat[i][j] = fish_save[4];
		...
		for (int i = 0; i < width; i++)
			for (int j = 0; j < height; j++)
				mat[i][j] = tmp_mat[i + 1][j + 1]; // copy of the tmp matrix in
		// the final matrix
	...
	// inverse transform
	public void inverse(double[] pix_d, byte[] pixels, double[][] matrix_big2,
			double[][][] wave3d, int width, int height, int m) {
		...
		for (int i = 0; i < width + m * 2; i++)
			for (int j = 0; j < height + m * 2; j++)
				res[i][j] = matrix_big2[i][j] + wave3d[i][j][0]
						+ wave3d[i][j][1] + wave3d[i][j][2] + wave3d[i][j][3]
						+ wave3d[i][j][4];
	...
	public void mirror(double[][] matrix_big, int width, int height, int m) {
		...
		for (int i = 0; i < width; i++) {
			for (int j = 1; j <= m; j++) {
				matrix_big[i + m][m - 1 + height + j] = matrix_big[i + m][m - 1
						+ height - j];
			}
		}
		...
		for (int i = 0; i <= m; i++) {
			for (int j = 0; j < height; j++) {
				matrix_big[m - 1 + width + i][m + j] = matrix_big[m - 1 + width
						- i][m + j];
			}
		}
		...
		for (int i = 0; i <= m; i++) {
			for (int j = 0; j <= m; j++) {
				matrix_big[i + width + m - 1][j + height + m - 1] = matrix_big[width
						+ m - 1 - i][height + m - 1 - j];
			}
		}
		...
		for (int i = 0; i <= m; i++) {
			for (int j = 0; j <= m; j++) {
				matrix_big[i + width + m - 1][j] = matrix_big[m - 1 + width - i][m
						* 2 - j];
			}
		}
		...
		for (int i = 0; i <= m; i++) {
			for (int j = 0; j <= m; j++) {
				matrix_big[i][j + height + m - 1] = matrix_big[2 * m - i][height
						+ m - 1 - j];
			}
		}
	...
	public double noise_estimate(ImageProcessor ip, int width, int height,
			double[] pix_d) {

		double stdev;

		ImagePlus med = NewImage.createByteImage("med image", width, height, 1,
				NewImage.FILL_BLACK);
	...
	public void run(ImageProcessor ip) {
		...
		// final image
		ImagePlus denoised = NewImage.createByteImage("Denoised image", width,
				height, 1, NewImage.FILL_BLACK);
		...
		ImagePlus simul = NewImage.createByteImage("Simulation", width, height,
		...
		a_trous_transform(matrix_big_s, matrix_big2_s, wave3d_s, width, height,
				m);
		...
		byte[] result = (byte[]) ip.getPixels();
		// display removed noise
		if (noise) {
			disp_noise(width, height, pix_d, noiseBuf);
			// copy to display
			for (int h = 0; h < pixels.length; h++)
				result[h] = noiseBuf[h];
		} else {
	...
	public int setup(String arg, ImagePlus imp) {
		...
		k3 = Double.parseDouble(args[2]);
		k4 = Double.parseDouble(args[3]);
		k5 = Double.parseDouble(args[4]);

		// Non gaussian noise removal
		poisson = Boolean.parseBoolean(args[5]);

		// Standard deviation
		pdev = Double.parseDouble(args[6]);

		// generate noise (if false remove noise)
		noise = Boolean.parseBoolean(args[7]);
	...
	public double stdev_calc(double[] tab) {
		...
		for (int n = 0; n < tab.length; n++) {
			sigma_t += (tab[n] - mean_t) * (tab[n] - mean_t);
		}
	...
	public void stdev_calc_m(double[][][] tab, double[] res, int width,
			int height, int m) {
