...
public class LaplacianFilterTileReader {
	...
	public static BasicTileReaderOutput processTile(BasicTileReaderInput input){
		...
		//re-scale the laplacian output -- not sure how to do this, or even if it's needed here
		//m1 = min(min(I2));
		//m2 = max(max(I2));
		//I2a = (I2-m1)/(m2-m1)*65535; ??
		tileImageLaplacian.getProcessor().convertToByte(true);


		//calculate the negative of the laplacian
		ImagePlus tileImageLaplacianNegative = tileImageLaplacian.duplicate();
		//ImageProcessor tileImageLaplacianNegativeProcessor = tileImageLaplacianNegative.getProcessor();
		//tileImageLaplacianNegativeProcessor.invert();
